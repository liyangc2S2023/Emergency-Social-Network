...
class FixOrder {
  static async createFixOrder(sender, comment, address, status) {
    const res = await FixOrderTable.find({ sender });
    if (res.length <= 0) {
      return FixOrderTable.create({
        sender, comment, address, status,
      });
    }

    const helper = '';
    return FixOrderTable.updateOne({ sender }, {
      $set: {
        comment, address, status, helper,
      },
    });
  }
  ...
  static async updateFixOrderByElectrian(sender, helper, status) {
    const res = await FixOrderTable.find({ sender }).sort({ timestamp: -1 }).limit(1);
    res.helper = helper;
    res.status = status;
    return FixOrderTable.updateOne({ sender }, { $set: { status, helper } });
  }

  static async getUnfixOrders() {
    return FixOrderTable.find({ status: { $in: ['needFix', 'fixing'] } }).sort({ timestamp: -1 });
